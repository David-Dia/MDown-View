...
- Markdown の見出し、リスト、リンク、画像、引用、表、タスクリスト、コードブロックに対応
- 見出しへのページ内アンカーナビゲーション
- Mermaid 図をローカルでレンダリング
- システム、ライト、ダークの外観モード
- ズーム操作を含む標準の編集・表示・ウインドウメニュー
- ウインドウのサイズと位置を記憶し、複数文書を見分けやすく配置
- 幅の広い表はウインドウに合わせて折り返し
- 幅広い文字エンコーディングに対応（UTF-8、UTF-16、GB18030、Latin-1）
...
アプリは macOS App Sandbox を使用し、ユーザーが選択したファイルに読み取り専用でアクセスします。プレビューページには厳格な Content Security Policy が適用され、スクリプトは同梱の Mermaid ライブラリのみ、リモート画像は `https` 経由でのみ読み込まれます。ネットワークアクセスは WebKit のためだけに存在し、解析やトラッキングには一切使用されません。
