...
- 记住窗口大小和位置，多文档窗口自动错开
- 宽表格自动换行以适应窗口
- 广泛的文本编码支持（UTF-8、UTF-16、GB18030、Latin-1）
...
应用启用了 macOS App Sandbox，只能以只读方式访问用户选择的文件。预览页面采用严格的内容安全策略：脚本仅来自内置的 Mermaid 库，远程图片只能通过 `https` 加载。网络访问仅供 WebKit 使用，绝不用于分析或跟踪。
